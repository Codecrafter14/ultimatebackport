...
public class BiomeSoulsandValley extends ElementsNetherMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			...
			this.spawnableCreatureList.add(new SpawnListEntry(EntitySkeleton.class, 20, 4, 10));
		}
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockSouldirt.block.getDefaultState().getBlock() || ground == Blocks.SOUL_SAND.getDefaultState().getBlock())
							&& (ground2 == BlockSouldirt.block.getDefaultState().getBlock()
									|| ground2 == Blocks.SOUL_SAND.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), Blocks.SOUL_SAND.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												|| state.getBlock() == Blocks.BONE_BLOCK.getDefaultState().getBlock()
												|| state.getBlock() == Blocks.BONE_BLOCK.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, Blocks.BONE_BLOCK.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.BONE_BLOCK.getDefaultState().getBlock()
									|| state.getBlock() == Blocks.BONE_BLOCK.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), Blocks.BONE_BLOCK.getDefaultState());
								if (genh > 0) {
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(-1, genh, 0)))
										this.setBlockAndNotifyAdequately(world, position.add(-1, genh, 0), Blocks.BONE_BLOCK.getDefaultState());
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(1, genh, 0)))
										this.setBlockAndNotifyAdequately(world, position.add(1, genh, 0), Blocks.BONE_BLOCK.getDefaultState());
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, -1)))
										this.setBlockAndNotifyAdequately(world, position.add(0, genh, -1), Blocks.BONE_BLOCK.getDefaultState());
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, 1)))
										this.setBlockAndNotifyAdequately(world, position.add(0, genh, 1), Blocks.BONE_BLOCK.getDefaultState());
								}
							}
		...
		private void addVines(World world, BlockPos pos) {
			this.setBlockAndNotifyAdequately(world, pos, Blocks.BONE_BLOCK.getDefaultState());
			int i = 5;
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.BONE_BLOCK.getDefaultState().getBlock()
					|| blockType == Blocks.BONE_BLOCK.getDefaultState().getBlock() || blockType == BlockSouldirt.block.getDefaultState().getBlock()
					|| blockType == Blocks.SOUL_SAND.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != Blocks.SOUL_SAND.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, Blocks.SOUL_SAND.getDefaultState());
		}
