...
import net.mcreator.nether.block.BlockBlackStone;
import net.mcreator.nether.block.BlockBasalt;
import net.mcreator.nether.ElementsNetherMod;

import java.util.Random;

@ElementsNetherMod.ModElement.Tag
public class BiomeBasaltDelta extends ElementsNetherMod.ModElement {
	@GameRegistry.ObjectHolder("nether:basalt_delta")
	public static final BiomeGenCustom biome = null;
	public BiomeBasaltDelta(ElementsNetherMod instance) {
		super(instance, 64);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Basalt Delta").setRainfall(0.5F).setBaseHeight(0.1F).setHeightVariation(0.2F).setTemperature(0.5F));
			...
			this.spawnableCreatureList.add(new SpawnListEntry(EntityMagmaCube.class, 20, 4, 4));
		}
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockBasalt.block.getDefaultState().getBlock() || ground == BlockBlackStone.block.getDefaultState().getBlock())
							&& (ground2 == BlockBasalt.block.getDefaultState().getBlock()
					...
					if (position.getY() < world.getHeight() - height - 1) {
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == Blocks.AIR.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockBasalt.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockBasalt.block.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == BlockBasalt.block.getDefaultState().getBlock()
